...
public class VelocityClassLoaderTest extends TestCase {
   ...
   * an {@code @AutoParcelGson} class. If Velocity loads its managers using the context class loader,
  ...
  public static class RunInClassLoader implements Runnable {
    ...
    public void run() {
      String source = Joiner.on('\n').join(ImmutableList.of(
          "package foo.bar;",
          "import " + AutoParcelGson.class.getName() + ";",
          "@AutoParcelGson abstract class Test {",
          "  abstract int baz();",
          "  static Test create(int baz) {",
          "    return new AutoParcelGson_Test(baz);",
          "  }",
          "}"));
